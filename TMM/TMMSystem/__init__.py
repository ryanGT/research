...
#            outname=rwkfortran.MakeFortranFunction(curfile,allparams,alldefs,compvars,defaultvalues,unknownparams,headername=headername)
    ...
    def GenMaxima(self,prefix='',skipMaxima=True,showbode=False, newsym=True, simplifycommands=[], **kwargs):
        ...
        See SymBodes for an explanantion of simplifycommands"""
        print('In GenMaxima, newsym='+str(newsym))
        if prefix:
        ...
        kwargs['simplifycommands']=simplifycommands
        if newsym:
    ...
    def IntegratedCurveFit(self, expmodname='', prefix='',
                           fitfortran=True, skipMaxima=False, **kwargs):
        """This function runs an automated system identification
        process for TMMSystem objects in three steps:

        1. Create Python and/or FORTRAN functions for the Bode
           responses of the system.  These functions will have the
           names prefix+'bode'+n, where the integer n is from
           iterating over the defined bodeouts of the TMMSystem.

        2. Prepare the experimental data for easy use with the cost
           function of the curve-fitting routine.  This is done by
           taking the name of a module contaiing experimental rwkbode
           instances, searching for matches to the input/output pairs
           defined in the bodeouts of the model, and creating a list
           of exp. bodes from these matches and saving the list to a
           new module.  Saved and loaded modules come from the
           scipy.io.save method that shelves a dictionary.

        3. Autogenerate a script that will actually run the the
           minimization that does the curve fitting.
        
        prefix will be used in a number of other places for filenames
        and such.
...
#        pdb.set_trace()
    ...
    #        pdb.set_trace()
        ...
        #        pdb.set_trace()
            ...
            if fitfortran:
                ...
                autoname=self.AutoGenerateFitFunction(savedname,\
                                                      fmodnames,\
                                                      functioname='bodevect',\
                                                      initialguesses=ig,\
                                                      prefix=prefix,\
                                                      fortran=True)
            else:
    ...
    def FindExpData(self, modulename, freqnames=['freq', 'expfreq', \
                                                 'f'], \
                    bodenames=['bodes','bode','expbodes','expbode', \
                               'avebodes'],\
                    datadir=None, prefix='exp'):
        ...
        elif type(modulename) == str:
            expdata = my_import(modulename)
        else:
            expdata = modulename#assume you passed in something
                                #useful, such as a
                                #txt_data_processing.Bode_Data_set
    ...
    def AutoGenerateFitFunction(self, savedname, bodenames, \
                                functioname=None, initialguesses=[], \
                                prefix='', fortran=False):
        ws=' '*4
        if prefix:
...
#        bodenames=['bode0','bode1']
        header='header.py'
        header_path = rwkos.FindFullPath(header)
        ...
        pylist = txt_mixin.txt_file_with_list()
        pylist.append_file_to_list(header_path)
        out=pylist.list.append
        ...
        out('')
        tail_path = rwkos.FindFullPath('autofitfunc_tail.py')
        pylist.append_file_to_list(tail_path)
        pylist.save(outname)
...
##         out(2*ws+'fi+=1')
##         out('')
##         out('def PlotAll(fitres, startfig=1):')
##         out(ws+'PlotExp()')
...
##         pylist.tofile()
    ...
    def SymDefineAllUs(self,aug=0,**kwargs):
        ...
        texname,frlist,basebodename,basecdname,debug,grind,optimize,skipMaxima,ratmx,simplifycommands=self.GetValuesorDefaults(kwargs,myoptions)
        mylist=latexlist([],texname)
        for elem in self.elemlist:
    ...
    def SymBodeAll(self,**kwargs):
        ...
        myoptions=['texname','frlist','basebodename','basecdname','debug','grind','optimize','skipMaxima','ratmx','simplifycommands']
        texname,frlist,basebodename,basecdname,debug,grind,optimize,skipMaxima,ratmx,simplifycommands=self.GetValuesorDefaults(kwargs,myoptions)
        print('In new sym')
        mylist=latexlist([],texname)
        alldefs=[]
        ...
        bodelines,filenames,chardetnames=self.SymBodes(basebodename=basebodename, basecdname=basecdname,debug=debug,simplifycommands=simplifycommands)
...
#        pdb.set_trace()
        for elem in self.elemlist:
            #curlines, curdefs, curparams,cursubs=elem.GetMaximaLatexString(aug=aug)
            curdefs = elem.GetDefs()
            curparams = elem.GetParams()
            cursubs = elem.GetSubs()
...
#        print('allsubs:'+str(allsubs))
    ...
    def SymBodes(self,basebodename='bodeoutputs',basecdname='chardet',myext='.txt',debug=False,optimize=True,ratmx=False,showbode=False,sub=True,simplifycommands=[],**unusedargs):
        ...
        simplifycommands is a list of lines which a %s in them where
        bodestr will be substituted to form a line like this:

        bodestr+':'+line%bodestr

        For example,

        line1='radcan(%s)'
        line2='divsim(%s,c*s+k)'
        simplifycommands=[line1,line2]

        would result in the following maxima lines be appended to the
        file if bodestr='bode0':
        bode0:radcan(bode0)
        bode0:divsim(bode0,c*s+k)"""
        if optimize:
            myext='.f'
        outfiles=[]
        linesout=[]
        ...
        for x,cb in enumerate(self.bodeouts):
            ...
            linesout.append('\\end{maxima-noout}')
            if simplifycommands:
                for line in simplifycommands:
                    linesout.append('\\begin{maxima-noout}')
                    cursub='\t'+bodestr+':'+line%bodestr
                    print('cursub='+cursub)
                    linesout.append(cursub)
                    linesout.append('\\end{maxima-noout}')
            linesout.append('\\begin{maxima-noout}')
            linesout.append('\t'+bodestr+':subst([sin=zsin,cos=zcos,sinh=zsinh, cosh=zcosh],'+bodestr+')')
            linesout.append('\\end{maxima-noout}')
            if showbode:
                substr=bodestr+'_sub'
                if sub:
...
#        pdb.set_trace()
    ...
    def GetValuesorDefaults(self,dictin,keylist):
        ...
        of values."""
        defaults={'N':self.maxsize,'texname':'SymBodeDev.tex','frlist':[],'basebodename':'bode','basecdname':'chardet','debug':False,'grind':True,'optimize':True,'skipMaxima':False,'ratmx':False,'radexpand':False,'simplifycommands':[]}
